...
class CAAEForm(FlaskForm):
    """Formulário para adicionar/editar CAAEs monitorados"""
    numero = StringField('Número do CAAE', validators=[DataRequired(), Length(min=5, max=20)])
    titulo = StringField('Título do Estudo')
    pesquisador = StringField('Pesquisador Principal')
    instituicao = StringField('Instituição')
    ativo = BooleanField('Ativo', default=True)
    submit = SubmitField('Salvar CAAE')
