...
from .forms import MonitorConfigForm, MonitorURLForm, TestEmailForm, RunMonitorForm, CAAEForm
...
@bp.route('/url/<int:url_id>/edit', methods=['GET', 'POST'])
@login_required
def edit_url(url_id):
...
@bp.route('/url/<int:url_id>/delete', methods=['POST'])
@login_required
...
# Rotas para gerenciamento de CAAEs
@bp.route('/caae')
@login_required
def caae_index():
    """Página de gerenciamento de CAAEs"""
    caae_list = CAAE.query.all()
    caae_logs = CAAELog.query.order_by(CAAELog.timestamp.desc()).limit(20).all()
    caae_form = CAAEForm()
    
    return render_template('monitor/caae.html', 
                          caae_list=caae_list,
                          caae_logs=caae_logs,
                          caae_form=caae_form,
                          title='Monitoramento de CAAEs')

@bp.route('/caae/add', methods=['POST'])
@login_required
def add_caae():
    """Adicionar novo CAAE para monitoramento"""
    form = CAAEForm()
    if form.validate_on_submit():
        # Verificar se o CAAE já existe
        existing_caae = CAAE.query.filter_by(numero=form.numero.data).first()
        if existing_caae:
            flash(f'O CAAE {form.numero.data} já está cadastrado.', 'warning')
            return redirect(url_for('monitor.caae_index'))
        
        caae = CAAE(
            numero=form.numero.data,
            titulo=form.titulo.data,
            pesquisador=form.pesquisador.data,
        ...
        log = CAAELog(
            caae=caae,
            tipo='configuracao',
            mensagem=f'CAAE adicionado para monitoramento: {caae.numero}'
        )
        db.session.add(log)
        
        db.session.commit()
        flash('CAAE adicionado com sucesso!', 'success')
    else:
        for field, errors in form.errors.items():
            for error in errors:
                flash(f'Erro no campo {getattr(form, field).label.text}: {error}', 'danger')
...
@bp.route('/caae/<int:caae_id>/edit', methods=['GET', 'POST'])
@login_required
def edit_caae(caae_id):
    """Editar CAAE monitorado"""
    caae = CAAE.query.get_or_404(caae_id)
    form = CAAEForm(obj=caae)
    
    if request.method == 'POST' and form.validate_on_submit():
        caae.numero = form.numero.data
        caae.titulo = form.titulo.data
        caae.pesquisador = form.pesquisador.data
        caae.instituicao = form.instituicao.data
        caae.ativo = form.ativo.data
        
        # Registrar no log
        log = CAAELog(
            caae=caae,
            tipo='configuracao',
            mensagem=f'CAAE editado: {caae.numero}'
        )
        db.session.add(log)
        
        db.session.commit()
        flash('CAAE atualizado com sucesso!', 'success')
        return redirect(url_for('monitor.caae_index'))
    
    return render_template('monitor/edit_caae.html', form=form, caae=caae, title='Editar CAAE')

@bp.route('/caae/<int:caae_id>/delete', methods=['POST'])
@login_required
def delete_caae(caae_id):
    """Excluir CAAE monitorado"""
    caae = CAAE.query.get_or_404(caae_id)
    numero = caae.numero
    ...
    log = CAAELog(
        tipo='configuracao',
        mensagem=f'CAAE removido: {numero}'
    )
    db.session.add(log)
    
    db.session.delete(caae)
    db.session.commit()
    
    flash(f'CAAE "{numero}" removido com sucesso!', 'success')
    return redirect(url_for('monitor.caae_index'))

@bp.route('/caae/check', methods=['POST'])
@login_required
def check_caae():
    """Verificar status dos CAAEs monitorados"""
    caae_list = CAAE.query.filter_by(ativo=True).all()
    if not caae_list:
        flash('Não há CAAEs configurados para monitoramento!', 'warning')
        return redirect(url_for('monitor.caae_index'))
    
    atualizacoes = []
    
    # Aqui seria implementada a lógica para verificar o status dos CAAEs na Plataforma Brasil
    # Como exemplo, vamos simular uma verificação
    for caae in caae_list:
        try:
            # Simulação de verificação (aqui seria implementada a lógica real)
            # Na implementação real, seria necessário fazer login na Plataforma Brasil
            # e consultar o status de cada CAAE
            
            # Registrar verificação no log
            log = CAAELog(
                caae=caae,
                tipo='verificacao',
                mensagem=f'Verificação manual realizada para CAAE {caae.numero}'
            )
            db.session.add(log)
            
            # Atualizar data da última verificação
            caae.ultima_atualizacao = datetime.utcnow()
            db.session.commit()
            ...
            log = CAAELog(
                caae=caae,
                tipo='erro',
                mensagem=f'Erro na verificação do CAAE {caae.numero}: {str(e)}'
            )
            db.session.add(log)
            db.session.commit()
    
    flash('Verificação de CAAEs concluída.', 'info')
    return redirect(url_for('monitor.caae_index'))

@bp.route('/caae/logs')
@login_required
def view_caae_logs():
    """Visualizar todos os logs de CAAEs"""
    page = request.args.get('page', 1, type=int)
    logs = CAAELog.query.order_by(CAAELog.timestamp.desc()).paginate(
        page=page, per_page=50, error_out=False)
    
    return render_template('monitor/caae_logs.html', logs=logs, title='Logs de CAAEs')
